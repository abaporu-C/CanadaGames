...
namespace CanadaGames.Models
{
    public class Athlete : Auditable, IValidatableObject
    {
        ...
        //Gets highest placement of Athlete
        public string HighestPlacement
        {
            get
            {
                ...
                if(this.Placements?.Count > 0)
                {
                    ...
                    {
                        int p = placement.Place;
                        if (p > highestPlacement) highestPlacement = p;
                    }
                }

                return highestPlacement switch
                {
                    1 => highestPlacement.ToString() + "st",
                    2 => highestPlacement.ToString() + "nd",
                    3 => highestPlacement.ToString() + "rd",
                    _ => highestPlacement.ToString() + "th",
                };                
            }
        }

        //Get Average placement of an Athlete
        public string AveragePlacement
        {
            get
            {
                ...
                if(this.Placements?.Count > 0)
                {
                    ...
                    {
                        count++;
                        averagePlacement += placement.Place;
                    }
                }

                averagePlacement /= count;
                int result = Convert.ToInt32(Math.Round(averagePlacement, 0));

                return result switch
                {
                    1 => result.ToString() + "st",
                    2 => result.ToString() + "nd",
                    3 => result.ToString() + "rd",
                    _ => result.ToString() + "th",
                };
            }
        }

        //Gets Lowest placements of an Athlete
        public string LowestPlacement
        {
            get
            {
                int lowestPlacement = 0;
                ...
                if(this.Placements?.Count > 0)
                {
                    ...
                    {
                        int p = placement.Place;
                        if (lowestPlacement == 0 || p < lowestPlacement) lowestPlacement = p;
                    }
                }

                return lowestPlacement switch
                {
                    1 => lowestPlacement.ToString() + "st",
                    2 => lowestPlacement.ToString() + "nd",
                    3 => lowestPlacement.ToString() + "rd",
                    _ => lowestPlacement.ToString() + "th",
                };
            }
        }

        //Gets Number of Events an Athlete participated.
        public int TotalEvents
        {
            get => this.Placements != null? this.Placements.Count : 0;            
        }

        //Get Number of Total Sports an athlete competes in
        public int NumOfSports
        {
            get
            {
                int sportsNum = 0;
                HashSet<string> sportsNames = new HashSet<string>();

                if(this.Placements?.Count > 0)
                {
                    foreach(Placement placement in this.Placements)
                    {
                        string sportName = placement.Event.Sport.Name;
                        if (!sportsNames.Contains(sportName)){
